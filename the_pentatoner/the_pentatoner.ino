...
void loop(void)
{
  ...
  fader1 = map(fader1, 0, 1023, 95, 24);    
  fader2 = analogRead(faderPin2);
  fader2 = map(fader2, 0, 1023, 95, 24);
  fader3 = analogRead(faderPin3);
  fader3 = map(fader3, 0, 1023, 95, 24);
...
//  }
  ...
  if (btn1.wasPressed()){
    // send note on
    Serial.print("Sending pitch "); Serial.println(currentNote1 + pitchShift);
    tempNote1 = fader1 % 12;
    currentNote1 = fader1;
    if (tempNote1 == 0 || tempNote1 == 2 || tempNote1 == 4 || 
          tempNote1 == 7 || tempNote1 == 9) {
      midi.send(0x90, currentNote1 + pitchShift, poti1);
    }
    else if (tempNote1 == 1 || tempNote1 == 3 || tempNote1 == 5 || tempNote1 == 10) {
      currentNote1 -= 1;
      Serial.print("Changed note to "); Serial.println(currentNote1 + pitchShift);
      midi.send(0x90, currentNote1 + pitchShift, poti1);
    }
    else if (tempNote1 == 6 || tempNote1 == 8 || tempNote1 == 11) {
      currentNote1 += 1;
      Serial.print("Changed note to "); Serial.println(currentNote1 + pitchShift);
      midi.send(0x90, currentNote1 + pitchShift, poti1);
    }
  ...
  if (btn1.wasReleased()) {
    midi.send(0x80, currentNote1 + pitchShift, 0);     // send note off
  }
  
  
  btn2.read();                    // button 2
  if (btn2.wasPressed()){
    tempNote2 = fader2 / 12; // floors current note since int
    Serial.print("fader is "); Serial.print(fader2); Serial.print(" floored is "); Serial.println(tempNote2*12);
    midi.send(0x90, (tempNote2 * 12) + pitchShift, poti2);
    midi.send(0x90, (tempNote2 * 12) + 4 + pitchShift, poti2);
    midi.send(0x90, (tempNote2 * 12) + 7 + pitchShift, poti2);
    //midi.send(0x90, fader2+12, poti2);
    currentNote2 = tempNote2*12;
  }
  if (btn2.wasReleased()){
    // send note off
    midi.send(0x80, currentNote2 + pitchShift, 0);
    midi.send(0x80, currentNote2 + 4 + pitchShift, 0);
    midi.send(0x80, currentNote2 + 7 + pitchShift, 0);
  ...
  if (btn3.wasPressed()){
    // send note on
    currentNote3 = fader3 / 12;     // floor current note since int
    currentNote3 *= 12;
    midi.send(0x90, currentNote3 - 3 + pitchShift, poti3);
    midi.send(0x90, currentNote3 + pitchShift, poti3);
    midi.send(0x90, currentNote3 + 4 + pitchShift, poti3);
    //midi.send(0x90, fader3 + 12, poti3);
  }
  if (btn3.wasReleased()){
    // send note off
    midi.send(0x80, currentNote3 - 3 + pitchShift, 0);
    midi.send(0x80, currentNote3 + pitchShift, 0);
    midi.send(0x80, currentNote3 + 4 + pitchShift, 0);
